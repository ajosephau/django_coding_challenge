...
from information.serializers import PersonWithFoodByTypeSerializer, CompanySerializer, MutualFriendsDetailSerializer, \
...
def mutual_friends_alive_with_brown_eyes(request, person_one_index, person_two_index):
    ...
    if request.method == 'GET':
        mutual_friends = Person.objects.alive_mutual_friends_brown_eyes(person_one, person_two)
        serializer = MutualFriendsDetailSerializer(person_one, context={'person_one':person_one, 'person_two':person_two, 'mutual_friends': mutual_friends})
        return Response(serializer.data)
    else:
        return Response(status=status.HTTP_400_BAD_REQUEST)
