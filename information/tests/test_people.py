...
class PeopleImportTestCase(TestCase):
    def setUp(self):
        company = Company.objects.create(index=0, name="TEST")
    ...
    def test_people_import_valid_case(self):
        input_data = [{'_id': '595eeb9cd4f669914b94721e', 'index': 0, 'guid': '882f83cd-e211-4fb1-aa19-114a086e2afc',
                       'has_died': False,
                       'balance': '$2,295.84', 'picture': 'http://placehold.it/32x32', 'age': 35, 'eyeColor': 'brown',
                       'name': 'Aisha Davis',
                       'gender': 'female', 'company_id': 1, 'email': 'aishadavis@earthmark.com',
        ...
        self.assertEqual(person_obj.friends_of.first(), person_obj) # check both sides of friendship relationship
        self.assertEqual(set(person_obj.favourite_foods.all().values_list('name', flat=True)), set(['strawberry', 'beetroot', 'carrot', 'celery']))
        self.assertEqual(set(person_obj.favourite_foods.all().values_list('type', flat=True)), set(['frt', 'veg', 'veg', 'veg']))
        self.assertEqual(set(person_obj.tags.all().values_list('name', flat=True)), set(['nulla', 'dolore', 'consequat']))

    def test_people_import_invalid_case(self):
        input_data = [{'_id': '595eeb9cd4f669914b94721e', 'company_id': 1}]
        issues, count = import_people_json(input_data)
        self.assertEqual(issues, ["Person: {'_id': '595eeb9cd4f669914b94721e', 'company_id': 1} missing 'index' key."])
