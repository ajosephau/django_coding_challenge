...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            # Import people data
            self.stdout.write(self.style.NOTICE(f'Starting to import people data...'))
            with open(people_json_path) as people_json_file:
                people_data = json.load(people_json_file)
                num_people = len(people_data)
                self.stdout.write(self.style.NOTICE(f'Estimated number of people to be imported: {num_people}'))

                people_issues, num_people = import_people_json(people_data)

                for idx, issue in enumerate(people_issues):
                    self.stdout.write(self.style.ERROR(f'Issue {idx+1}/{len(people_issues)}: {issue}'))
                if people_issues:
                    raise CommandError(f"{len(people_issues)} issues detected: stopping import.")

                self.stdout.write(self.style.SUCCESS(f'Successfully imported {num_people} people.'))